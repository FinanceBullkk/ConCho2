...
const { updateActiveRow, deleteActiveRowsWhere, addAllowedTimeSlot } = require('../pg-test-utils');
...
beforeAll(async () => {
  ...
  office = await fx.createOffice({ name: 'Golden Office', code: 'GOLD' });
});
...
describe('Golden path — core L&D loop end to end', () => {
  test('program → enrol → schedule → attendance → completion → certificate → verify', async () => {
    ...
    await updateActiveRow('Class', seed.class1._id, { programId });
