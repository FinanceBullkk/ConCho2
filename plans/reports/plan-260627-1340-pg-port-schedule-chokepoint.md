...
- **S3 — schedule txn-aware methods + scheduleService blocks.** SPLIT into S3a + S3b (owner 2026-06-27: repo-layer first, design-doc the orchestration before cutover).
  ...
  - **S3b — orchestration cutover.** Design doc done: `plans/reports/design-260627-1601-pg-s3b-schedule-orchestration-cutover.md` (decisions: P1=`SELECT…FOR UPDATE`, P2=one `insertSession`, SPLIT into S3b-1 create/cancel + S3b-2 updateSchedule after S4).
    ...
    - **S3b-2 — `updateSchedule` cutover.** ✅ DONE 2026-06-27. Ported the last 2 deferred repo methods to pg — `updateScheduleById` (field-mapped UPDATE: core columns + meta-extras merge; empty data → no-op returning the row) + `findTeamById` (opts-select: classId / members-with-status) — both mongo now use `sessionOf`. Cut `updateSchedule` (use-cases) over from `startSession`→`runInTransaction` (threads `tx`; the only direct-Mongoose was already via repo). **Schedule repo selector is now a CLEAN SWAP** (every method has a pg twin — no mongo-only remainder). Parity `schedule-update-team.pg.test.js` 6/6 on Neon; mongo-default reassign/usecases 20, broad sweep (booking/race/cancel/room/teams/learning/waitlist/staleReconcile/mode) 107; full pg-parity 42 suites/262. DB_BACKEND=mongo default unchanged.
    - **Remaining (read-path completion):** the post-commit / pre-tx Mongo-direct READS in the create/cancel paths (bookSlot/bookCohortSlot/adminCreate re-fetch populate, cancelSlot initial populated load + waiter `User.find`) — route through dual-backend repo reads so `DB_BACKEND=postgres` runs the whole booking flow end-to-end. Lower-risk (reads), harmless while mongo-default.
