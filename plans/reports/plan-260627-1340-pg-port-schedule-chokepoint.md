...
- **S3 — schedule txn-aware methods + scheduleService blocks.** SPLIT into S3a + S3b (owner 2026-06-27: repo-layer first, design-doc the orchestration before cutover).
  ...
  - **S3b — orchestration cutover (PENDING DESIGN DOC first).** Port `updateScheduleById` (generic field-mapper) + `findTeamById` (opts-session), then migrate `bookSlot`/`bookCohortSlot`/`adminCreate`/`cancelSlot` + `updateSchedule`/`deleteSchedule` from `startSession`→`runInTransaction`, replace direct `Schedule.create`/`Team.findByIdAndUpdate` with dual-backend seams. **Design doc required first** — the Mongo write-lock (`findByIdAndUpdate {updatedAt}` serialize trick) has NO clean PG equivalent (PG = unique-index + row-lock); the booking-write seam must extend to officeId/capacity/topic/spread fields.
