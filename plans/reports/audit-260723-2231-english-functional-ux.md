...
| U2 | Med | Live operational loop (create Meeting → mark attendance → eligibility → level) is built + unit/e2e-tested but has ~0 real usage — all 984 sessions are imported evidence; attendance save is live-only. | **CLOSED 2026-07-24** — loop driven end to end in a real browser against the real imported workbook. It was NOT shippable: level entry 500'd on every first entry (row written, audit lost) and seeded environments had no levels at all. 3 defects fixed + regression test. Report: [`validation-260724-0303-english-live-loop-real-data.md`](validation-260724-0303-english-live-loop-real-data.md). Still open: real-operator (owner) usage on live sessions. |
