...
| 19 | `services/audit-repository` (**Wave-E slice E1 — audit write path**) | `findChainHead` + `insertChainedRow` + `findChainWindow` under auditService's serialized append queue; `verifyChain({repo})` drives either backend | audit_log (**new — migration 029**; partial-unique `seq` chain-fork guard, append-only: no soft-delete/updated_at, created_at index for the E2 retention purge — PG has no TTL, Mongo keeps 730d) | identical hashes across ObjectId/Date ⇄ text/ISO row shapes · tamper → hash-mismatch + deleted row → missing-rows, same verdicts/seq both backends · dup-seq 23505⇄11000 · entity/actorRole enum parity from the SAME schema enumValues (ValidationError on both) | (this) |
