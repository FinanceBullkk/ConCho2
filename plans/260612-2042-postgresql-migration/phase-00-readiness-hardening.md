...
| 0.3 | Extract `searchService` model access → `repository` | WS-A | refactor | Low | ✅ yes (self-contained) |
| 0.4 | Extract `lib/branding.js` + `routes/auditRoutes.js` reads → repository | WS-A | refactor | Low | ✅ yes |
| 0.5 | Extract `services/export/*` aggregations → repository | WS-A | refactor | Low-Med | ✅ yes |
