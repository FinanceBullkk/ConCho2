...
|----|-----|-------|-----------|----------|
| DATA-016 | P3 | 02 | No reconcile check for stale `waiting` waitlist rows on past sessions — add check #12 (flag or auto-expire) | **FIXED 2026-06-12** (ops/data sweep): read-only check #12 `stale_waitlist_entry` (`reconcile/waitlist-checks.js`) flags waiting rows on past/cancelled/deleted sessions (flag-only — reconcile never mutates); model enum + summary + client meta/i18n (incl. missing `orphan_room_booking` label); 5 integration tests |
...
| OPS-010 | P2 | 05 | Sentry cron missed-run detection not armed for pinger-driven runs — CRON_JOBS entries carry no `schedule`, external check-ins create schedule-less monitors. Fix: add crontab to CRON_JOBS (reconcile `0 2 * * *`, attendance `0 * * * *`, assignment `0 1 * * *` per owner) + pass through cronRoutes | **FIXED 2026-06-12** (ops/data sweep): all 3 CRON_JOBS entries carry their crontab (flows through cronRoutes automatically; in-process reconcile still overrides with `RECONCILE_CRON`); schedule-validity unit test |
| OPS-011 | P2 | 05 | envValidator misses README-required `CORS_ORIGINS`/`CLIENT_ORIGIN` (prod boots → runtime write-outage / localhost reset links); README §6.4 misses boot-required `IMPORT_DEFAULT_PASSWORD` | **FIXED 2026-06-12** (ops/data sweep): both added to `REQUIRED_IN_PRODUCTION` (+4 unit tests); README §6.4 boot-required notes (IMPORT_DEFAULT_PASSWORD row was already added in round 8) |
| OPS-012 | P3 | 05 | cron `?token=` leaks into pino `req.url` + 730-day audit notes (redact is key-based, can't mask URL substrings). Fix: redact token query param at cronAuth log/audit sites (owner picked redact over dropping query support) | **FIXED 2026-06-12** (ops/data sweep): new `lib/redact-url-token.js` applied at the pino-http req serializer + all 3 cronAuth log/audit sites; helper + audit-note redaction tests |
...
| CODE-017 | P3 | 07 | 18/20 lazy requires (`controllers/auth/*` — User/mfaService/invalidateUserCache per handler) likely no longer dodge a real cycle after the modular split. Hoist opportunistically when touching those files; 2 legit cycles stay (completion repo, waitlist→scheduleService) | **FIXED 2026-06-12** (deps-light round): all 17 auth-controller lazy requires hoisted; no cycle; full suite green. The 2 legit cycles untouched |
| DEPS | P3 | 07 | Pending majors, each its own post-audit task: express 4→5, mongoose 8→9 (after PostgreSQL gate decision), eslint 9→10 (ratchet re-baseline), bcryptjs/uuid/dotenv batch; googleapis 146→173 only with calendar retest | **PARTIAL 2026-06-12:** bcryptjs 3 + dotenv 17 done; uuid DROPPED (→ `crypto.randomUUID()`, uuid ≥13 ESM-only); **express 4→5 DONE** (in-place sanitize middleware + `/{*splat}` SPA fallback + req.body `{}` shim; 902/902 green; +8 compat tests). eslint 10 BLOCKED on `eslint-plugin-jsx-a11y` peer ≤9 — recheck on plugin update. mongoose 9 stays gated on PostgreSQL decision; googleapis needs calendar retest |
