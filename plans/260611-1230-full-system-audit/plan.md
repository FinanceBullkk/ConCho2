...
| DEPS | P3 | 07 | Pending majors, each its own post-audit task: express 4→5, mongoose 8→9 (after PostgreSQL gate decision), eslint 9→10 (ratchet re-baseline), bcryptjs/uuid/dotenv batch; googleapis 146→173 only with calendar retest | **PARTIAL 2026-06-12:** bcryptjs 3 + dotenv 17 done; uuid DROPPED (→ `crypto.randomUUID()`, uuid ≥13 ESM-only). eslint 10 BLOCKED on `eslint-plugin-jsx-a11y` peer ≤9 — recheck on plugin update. express 5 pre-scouted (only `app.get('*')` + express-mongo-sanitize need changes; `validate` already getter-safe). mongoose 9 stays gated on PostgreSQL decision; googleapis needs calendar retest |
