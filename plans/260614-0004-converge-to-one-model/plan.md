...
| 4 | UX journeys 🟡 | re-cut sidebar groups into persona journeys; collapse English↔Operations once storage unified. **Slice 1 done (2026-06-18):** Learning→Cohorts is now a UNIFIED catalog (mode="all") listing BOTH worlds with a deliveryType column + world filter; per-row actions gate by deliveryType (ends the "Cohorts tab looks empty" confusion). Remaining: fold the English section in / persona-journey sidebar. | med |
