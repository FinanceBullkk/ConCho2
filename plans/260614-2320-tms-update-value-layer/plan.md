...
## Progress
> **Phase 1 COMPLETE** (8/8 slices) — 2026-06-15. All shipped on `feat/tms-update-value-layer`, suite green (client 344, server learning/notif green), lint 63 (cap), build clean. Not yet pushed.
...
- ✅ **S8 — Home alert wiring + drill close-the-loop** (2026-06-15). QuickActions' **overdue** + **certs** tiles now drill straight to the filtered learner list for admins (`/reports/drill?metric=overdue|expiring`); non-admins keep the broader list destinations (drill is Admin-only). Fixed `AlertBand`'s **stale legacy routes** (`/operations?tab=attendance` → `/calendar?tab=attendance`; `/academy?tab=teams` → `/english?tab=teams`) that previously redirected and dropped their query. ⌘K is already a working cross-entity command palette (`SearchPalette`) — left as-is. Tests: +1 client (admin drill case). Suite 344✓, lint 63, build clean. No backend.
- ✅ **S7 — Notification center + delivery prefs** (2026-06-15). `/notifications` (any authenticated, self-scoped); bell gains a "View all" link. Feed with category filter (derived from NotificationLog `type`) + mark-all-read, and a **delivery preferences** panel: per-category in-app/email toggles + daily/weekly/off digest. Backend: `User.notificationPreferences` (Mixed) + `GET`/`PUT /notifications/preferences` (`notification.read`, self-scoped, audited; partial PUT merges over server defaults; zod string-keyed record validated against the category list). New api bindings + `useNotificationPreferences`/`useSetNotificationPreferences`. Tests: +3 server integration (defaults / partial-merge+persist / invalid-digest 400), +3 client. Client 343✓, server notif 9✓, lint 63, build clean.
