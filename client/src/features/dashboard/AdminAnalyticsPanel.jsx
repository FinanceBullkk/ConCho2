...
// ──────────────────────────────────────────────────────────
// AdminAnalyticsPanel — IA cleanup (2026-06-13)
// The heavy training-analytics body that used to live on the Home dashboard.
// Now mounted as Reports▸Overview so all numbers live in one section; Home is
// a lightweight landing. Admin-only data (the stats endpoint is Admin-gated;
// the hooks stay disabled until the caller is an Admin past mustChangePassword).
// ──────────────────────────────────────────────────────────
export default function AdminAnalyticsPanel() {
  ...
  // ── Data fetching ────────────────────────────────────
  // Mirror the old Home gate: don't fire while the forced-password-change
  // modal owns the screen (every call 403s on the mustChangePassword gate).
  ...
  // Only show the full spinner on FIRST load (no data yet).
  ...
  // When refetching after a filter change, dim the content slightly.
  const isRefreshingFilters = isFetching && !loadingStats;

  return (
    <div className="space-y-5" aria-busy={isFetching}>
      {/* Refresh control (top-right) — replaces the old PageHeader actions */}
      ...
      {/* dim/disable everything BELOW the filter bar while a new filter query is in flight */}
