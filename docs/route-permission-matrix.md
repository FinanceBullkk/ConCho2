...
| `/api/english-training` | Admin/Coordinator (`report.read`) | targeted employee BU/job-role correction AND exam-result entry (`POST`/`DELETE /enrollments/:id/exam-result`): Admin/Coordinator + `enrollment.manage`; import is ops CLI only | feature-flagged historical English operations surface. Sessions/attendance/eligibility are read-only; exam result (level) is a write, gated ≤2 absences server-side; raw workbook evidence is append-only |
