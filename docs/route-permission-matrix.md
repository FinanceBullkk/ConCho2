...
| `/api/english-training` | Workspace overview/live eligibility: Admin/Coordinator/Teacher (`report.read`/`attendance.read`) with assigned-Teacher resource scope; managed learners + teachers: Admin/Coordinator; Archive/status/history: Admin/Coordinator | managed learner/provision: Admin/Coordinator `enrollment.manage`; live level: Admin/Coordinator/assigned Teacher `english.evaluate`; archive cutover: Admin `settings.manage`; historical writes only before freeze | English Operations composition plus read-only historical Archive. Participant denied. Cutover is audited and one-way in app; application guards + PostgreSQL triggers reject all `eng_*`/raw writes/import after freeze |
