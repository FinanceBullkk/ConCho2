...
| `/api/english-training` | Workspace overview, canonical class/course/run/session reads, and live attendance roster: Admin/Coordinator (`report.read` or `attendance.read` by route); managed learners: Admin/Coordinator `enrollment.manage`; imported evidence reads: Admin/Coordinator `report.read` | stable class + PIC + first Course Run: `cohort.manage`; learner start, active learner leave, and cross-class learner transfer: `enrollment.manage`; Meeting + Session Unit create: `session.book`; atomic full-roster P/A save: `attendance.mark`; corrections and categorical level writes retain their declared capabilities | English is a dedicated canonical module. Leave and transfer are soft historical transitions: transfer links the source Enrollment/Membership to one new active target chain while retaining roster/attendance/evaluation history and atomically auditing the intent. Ordinary capacity is enforced; capacity override is separate. No generic Program/Class/PIC-Team handoff or Archive cutover endpoint. Participant denied. Teacher access to canonical rosters/writes remains closed until assigned-resource scope is ported. Raw workbook/import evidence stays guarded; operational `eng_*` writes require validated, transactional, audited commands. |
