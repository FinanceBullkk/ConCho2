...
| `/api/english-training` | Admin/Coordinator (`report.read`) | targeted employee BU/job-role correction: Admin/Coordinator + `enrollment.manage`; import is ops CLI only | feature-flagged historical English operations surface. Sessions/attendance/eligibility are read-only; raw workbook evidence is append-only |
