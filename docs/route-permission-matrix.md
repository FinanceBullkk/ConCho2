...
| `/api/english-training` | Workspace overview and canonical English class/course/roster reads: Admin/Coordinator `report.read`; managed learners: Admin/Coordinator `enrollment.manage`; imported evidence reads: Admin/Coordinator `report.read` | atomic stable class + current PIC + first Course Run: Admin/Coordinator `cohort.manage`; employee corrections and categorical level writes retain their declared capabilities | English is a dedicated canonical module. No generic Program/Class/PIC-Team handoff or Archive cutover endpoint. Participant denied. Teacher access to canonical rosters/writes remains closed until assigned-resource scope is ported. Raw workbook/import evidence stays guarded; operational `eng_*` writes require validated audited commands. |
